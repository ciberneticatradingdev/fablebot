...
#   2. voice mixer  → /tmp/fablebot-voice.pcm FIFO
...
FIFO="${LIVE_FIFO:-/tmp/fablebot-voice.pcm}"
...
( cd "$HERE/chat" && nohup node poller.mjs > /tmp/fablebot-chat.log 2>&1 & echo "  pid $!" )

echo "[rig] voice mixer → $FIFO"
[ -p "$FIFO" ] || mkfifo "$FIFO"
( nohup python3 "$HERE/voice/live-mix.py" > "$FIFO" 2>/tmp/fablebot-mix.log & echo "  pid $!" )
